...
function add_hospital($hospitalname, $description, $doctors, $address, $longitude, $latitude, $status){
		//Require Databse config file
		require 'config.php';

		//add admin
		$stmt = $mysqli->prepare("INSERT INTO hospitals(hospitalname, description, doctors, address, longitude, latitude, status) VALUES(?,?,?,?,?,?,?)");
		$stmt->bind_param("ssissss",$hospitalname, $description, $doctors, $address, $longitude, $latitude, $status);
		if($stmt->execute()){
			return true;
		}else{
			return false;
...
function update_hospital($hid, $hospitalname, $description, $doctors, $address, $longitude, $latitude){
		//Require Databse config file
		require 'config.php';
		//fetch all user
		$stmt = $mysqli->prepare("UPDATE hospitals SET hospitalname = ?, description = ?, doctors = ?, address = ?, longitude = ?, latitude = ? WHERE id = ?");
        $stmt->bind_param("ssisssi",$hospitalname, $description, $doctors, $address, $longitude, $latitude, $hid);
        if($stmt->execute()){
            return true;
        }else{
            return false;
